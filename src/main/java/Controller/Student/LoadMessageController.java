...
public class LoadMessageController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		request.setCharacterEncoding("utf-8");
			...
			System.out.println("ssssssssssss"+request.getParameter("nhanvien"));
			HttpSession session = request.getSession();
			String mString=(String) session.getAttribute("user");
			ArrayList<Chat> chat=null;
			chat =chatBO.getChatList("10218");
			if(request.getParameter("nhanvien").equals(null)) {
				 chat =chatBO.getChatList("10218");
				 System.out.println("ttttttt"+request.getParameter("nhanvien"));
			}
			else {
			...
			for (Chat c:chat) {
				...
				if(mString.equals(c.getNguoiGui())) {
